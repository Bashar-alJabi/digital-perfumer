...
export default function TechStackSection() {
	...
	const activeGroup = skillGroups.find((g) => g.id === activeGroupId)!;

	return (
		<section className="relative py-32 md:py-44 px-6 md:px-12 bg-perfume-bg text-perfume-text border-t border-perfume-soft/40 select-none">
			<div className="max-w-7xl mx-auto">
				...
				<div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 md:mb-20">
					<div>
						...
						<h2 className="mt-4 text-4xl md:text-6xl lg:text-7xl font-serif font-normal text-perfume-text">
							The Digital <br />
					...
					<p className="text-sm md:text-base font-light text-perfume-text/75 max-w-md font-sans leading-relaxed">
						Every creation requires precise raw materials. Here is
						the technical inventory used to craft memorable digital
						products.
					</p>
				</div>
						...
						{skillGroups.map((group) => {
							...
							return (
								<button
									...
									className={`w-full text-left p-5 md:p-6 rounded-2xl border transition-all duration-300 flex items-center justify-between ${
										isActive
											? "border-perfume-primary bg-perfume-surface shadow-md"
											: "border-perfume-soft/60 bg-perfume-bg/50 hover:border-perfume-primary/30"
									}`}
								>
									<div className="flex items-center gap-4">
										...
										<div>
											...
											<p className="text-xs font-light text-perfume-text/60 font-sans">
												{group.skills.length}{" "}
												Ingredients
											</p>
										</div>
									...
									<span
										className={`text-lg transition-transform duration-300 ${isActive ? "text-perfume-primary translate-x-1" : "text-perfume-text/30"}`}
									>
										→
									</span>
								</button>
					...
					<div className="lg:col-span-7">
						<AnimatePresence mode="wait">
							...
							>
								<div>
									...
									<div className="flex flex-wrap gap-3">
										{activeGroup.skills.map((skill) => (
											<span
												key={skill.name}
												className={`text-sm md:text-base font-sans px-4 py-2 rounded-xl transition-all duration-300 border ${
													skill.highlight
														? "bg-perfume-primary/10 border-perfume-primary/40 text-perfume-text font-medium shadow-sm"
														: "bg-perfume-bg border-perfume-soft/80 text-perfume-text/80"
												}`}
											>
												{skill.name}
											</span>
								...
								<div className="mt-12 pt-6 border-t border-perfume-soft/40 flex items-center justify-between text-xs font-mono text-perfume-text/40">
